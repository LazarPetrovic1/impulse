...
function Chat(props) {
  ...
  useEffect(() => {
    socket.emit('getChat', { userId: user && user._id, theirId: selected && selected.user && selected.user })
    if (!chat) {
      socket.emit('getChat', { userId: selected && selected.user && selected.user, theirId: user && user._id })
    }
  ...
  const onMessageSubmit = (e) => {
    ...
    } else if (selected && selected.user && !chat._id) {
        ...
        {chat && chat.messages && chat.messages.map(message => (
          <div key={message._id}>
            <div className={`d-flex my-3 ${message.user === user._id && "justify-content-end"}`}>
              <span style={{ borderRadius: '10px' }} className={`px-3 py-2 ${message.user === user._id ? "bg-primary" : "bg-secondary"}`}>{message.body}</span>
