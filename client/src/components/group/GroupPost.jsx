  ...
  return user && group ? (
    ...
    >
      <div className="d-flex">
        <img src={`https://robohash.org/${user._id}?set=set4&size=40x40`} alt={`${user.firstName} ${user.lastName}`} />
        <h2 className="ml-3">
          <Link to={`/social/profile/${user._id}`}>{user.firstName} {user.lastName}</Link>&nbsp;
          <i className="fas fa-caret-right" style={{ color: "rgb(0, 123, 255)", fontSize: "30px" }} />&nbsp;
          <Link to={`/groups/${group._id}`}>{group.name}</Link>
        </h2>
      </div>
      <article className="mt-4 mb-2">
        {post.body && <MarkdownRenderer markdown={post.body} />}
        {post.isMedia && post.media.map(med => (
      ...
      >
        <article>
          ...
          <div className="d-flex justify-content-between mx-5 my-3">
            <button className="btn btn-success btn-lg" onClick={removePost}>Yes</button>
            <button className="btn btn-danger btn-lg" onClick={onClose}>No</button>
          </div>
        </article>
      </Modal>
    </section>
