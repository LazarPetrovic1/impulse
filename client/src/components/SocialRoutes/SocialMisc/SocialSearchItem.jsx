...
const { _male, _female, _hidden } = socialsearchitemcomponent;

function SocialSearchItem({ fs, src }) {
  const { language } = useContext(LanguageContext);
  const { firstName, lastName, username, sex, city, country, _id } = fs;
  return (
    <div
      className="border d-inline-block p-3"
      style={{ pointerEvents: "all", marginLeft: "50px" }}
    >
