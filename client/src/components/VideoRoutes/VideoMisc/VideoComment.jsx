...
import { videoDeleteComment, videoReplyToComment, videoEditComment } from '../../../actions/video';
import Moment from 'react-moment';
import PropTypes from 'prop-types';
import { connect } from 'react-redux';
import VideoCommentReply from './VideoCommentReply';

function VideoComment({ comment, videoId, videoDeleteComment, videoReplyToComment, auth: { user }, videoEditComment }) {
  ...
  return (
    <section className="ml-auto my-5 position-relative" style={{ pointerEvents: "all" }}>
      <div>
        <div className="d-flex align-items-center">
          ...
          <h3 className="m-0">
            <Link style={{ textDecoration: "none", color: "#eee" }} to={`/social/profile/${comment.user}`}>
              @{comment.name}
            </Link>
          </h3>
        </div>
      ...
      {comment.user === user._id && (
        <div style={{ top: "1rem", right: "1rem" }} className="position-absolute">
          {isReplying ? (
            <button onClick={() => setIsReplying(false)} className="btn btn-dark btn-lg mx-2">
              <i className="fas fa-times" />
            </button>
          ) : (
            <button onClick={() => setIsReplying(true)} className="btn btn-primary btn-lg mx-2">
          ...
          <button className="btn btn-danger btn-lg mx-2" onClick={() => videoDeleteComment(videoId, comment._id)}><i className="fas fa-trash" /></button>
        </div>
      )}
      {isReplying && (
...
export default connect(mapStateToProps, { videoDeleteComment, videoReplyToComment, videoEditComment })(VideoComment);
