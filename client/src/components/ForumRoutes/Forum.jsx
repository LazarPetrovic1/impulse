...
import { getAllForumPosts, forumPostDismiss, deleteForumPost, saveForumPost } from '../../actions/forum'
...
function Forum ({
  ...
  const search = (text) => {
    ...
    else posts.filter(post => post.body.toLowerCase().includes(text.toLowerCase()))
  }
  ...
  const nonDismissedPosts = posts.filter(post => !user.dismissedPosts.includes(post._id))
      ...
      <ForumGrid>
        ...
        <div className='span-col-8 my-5'>
          {nonDismissedPosts.map((post) => (
            <div
              key={post._id}
              className={isDarkTheme && 'bg-overlay'}
            >
              ...
              <p className='text-right text-secondary lead mr-2'>- by {post.author}</p>
              <div className='text-right my-3'>
                ...
                {post.savedBy.filter(sb => sb.user === user._id).length > 0 ? (
                  <button
