...
router.delete("/public/:id", async (req, res) => {
  try {
    ...
    const newPosts = await Array.from(posts).filter(p => p.id.toString() !== req.params.id.toString())
    res.json(newPosts)
  } catch (e) {
    console.error(e.message)
    if (e.kind === 'ObjectId') return res.status(404).json({ msg: 'Post not found' })
...
router.put("/public/comment/:id/:comment_id/:uid", async (req, res) => {
  try {
    ...
    if (comment.user.toString() !== req.params.uid) return res.status(401).json({ msg: 'User not authorised.' })
    const newComment = {
      ...comment,
      content,
    }
...
router.delete("/public/comment/:id/:comment_id/:uid", async (req, res) => {
  try {
    ...
    if (!comment) return res.status(404).json({ msg: 'Comment not found.' })
    if (comment.user.toString() !== req.params.uid) return res.status(401).json({ msg: 'User not authorised.' })
    const removeIndex = post.comments.map(comment => comment.user.toString()).indexOf(req.params.uid)
...
router.post(
  "/public/comment/:id/:comment_id/:uid/reply",
  async (req, res) => {
    try {
      const user = await User.findById(req.params.uid).select("username")
      const post = await ForumPost.findById(req.params.id)
...
router.get("/public/comment/:id/:comment_id/reply", async (req, res) => {
  try {
    const post = await ForumPost.findById(req.params.id)
    const comment = await post.comments.find(comm => comm.id.toString() === req.params.comment_id)
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      if (reply.user.toString() !== req.params.uid) return res.status(401).json({ msg: 'User not authorised.' })
