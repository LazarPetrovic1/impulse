...
async function uploadVideo(req, res) {
  try {
    ...
    const uploadResponse = await cloudinary.uploader.upload(fileStr, { resource_type: "video" })
...
async function getMyVideos(req, res) {
  try {
    const posts = await VideoPost.find({ user: req.user.id }).sort({ date: -1 });
    res.json(posts)
  } catch (e) {
    console.error(e.message);
...
async function getUsersVideos(req, res) {
  try {
    const posts = await VideoPost.find({ user: req.params.id }).sort({ date: -1 });
    res.json(posts)
  } catch (e) {
    console.error(e.message);
    res.status(500).json({ msg: "Internal server error." });
...
async function impulseVideo(req, res) {
  try {
    ...
    if (
      ...
      await post.save();
      return res.json(post.impulsions);
    }
